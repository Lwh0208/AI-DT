...
本系统通过分析历史聊天数据，自动沉淀多角色的原生画像与主题记忆；在运行阶段结合多方画像、当前聊天窗口最近 20 条上下文消息，高精度预测张照西的回复；并在模拟一天结束时进入 Dream 模式更新会话中涉及角色的画像与记忆，使助手的回复行为逐步接近本人。
...
| 测试数据 | `data/test/` | 模拟运行与 Dream 更新 | 每次模拟测试 |

- **历史数据**只在画像初始化阶段读取，默认仅使用 `PROFILE_INIT_CUTOFF` 之前的消息生成初始画像；多个文件会按聊天窗口分块注入，避免不同私聊被混成同一条会话
- **测试数据**在模拟运行阶段按聊天窗口逐文件处理，触发回复预测和 Dream 模式；当前窗口的最近上下文会独立记录
- 两种数据互不干扰，可以独立更新
...
│   └── 张照西/
│       └── 2026-03-01/
│           ├── profile.md    # 该日期可用的完整用户画像
│           └── memory.md     # 该日期可用的完整个人记忆库
├── runtime/                  # 运行时数据
...
```
张照西 2025/9/1 12:01:56
赵博，啥情况

赵宇 2025/9/1 14:10:05
毕然他们好像要投资源，本地部署GLM5
```
...
直接放在 `data/test/` 根目录下的 `.txt` 会按单聊兼容处理。单聊中所有非张照西消息都会触发模拟回复；群聊中只有提及张照西或其别名的消息才触发模拟回复。
...
- 单聊中所有非张照西消息都会触发回复预测
- 群聊中只有提及张照西/照西/西哥的消息才触发回复预测
- 张照西主动发起的消息和真实回复会进入当前窗口上下文，但不会触发预测
- 完整运行结束后会生成 `runtime/transcript_*.txt`，用于查看测试消息、预测回复、真实回复对照
- 日期变更时强制触发 Dream 模式，按全局时间线综合当天所有窗口，更新当天会话中涉及到的所有角色画像与记忆
- 画像与记忆按日期版本化存储；预测某日回复时读取该时间点之前最新的角色画像版本
- 更新时间为对话时间而非真实时间
